...
async function verifyMasterchainBlock(
    ...
    let existingData = {};
    if (fs.existsSync('tests/testnet/keyblock' + keyBlockNumber + '.json')) {
        existingData = JSON.parse(fs.readFileSync('tests/testnet/keyblock' + keyBlockNumber + '.json', 'utf8'));
    }
    const mergedData = { ...existingData, ...signaturesData };
    fs.writeFileSync('tests/testnet/keyblock' + keyBlockNumber + '.json', JSON.stringify(mergedData, null, 2));
    console.log('Signatures written to tests/testnet/keyblock' + keyBlockNumber + '.json');
}
...
(async () => {
    ...
    while (true) {
        ...
        if (counter === 0) {
            ...
            fs.writeFileSync('tests/testnet/keyblock' + keyBlockNumber + '.json', JSON.stringify(testData, null, 2));
            console.log('Block data written to tests/testnet/keyblock' + keyBlockNumber + '.json');
            break;
        }
        counter--;
        const keyBlockInfo = await client.getFullBlock(parsedBlock.info.prev_key_block_seqno);
    ...
    fs.writeFileSync('tests/testnet/keyblock' + keyBlockNumber + '.json', JSON.stringify(mergedData, null, 2));
    console.log('Block header written to tests/testnet/keyblock' + keyBlockNumber + '.json');
